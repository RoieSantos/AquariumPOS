-- Skips creating a Payroll Run line entirely for an employee with zero timesheet entries in the
-- period, instead of creating one that just computes to (near) zero pay. Per: "once the timesheet
-- create payroll run dont create run for employee dont have timesheets."
--
-- This applies to any admin_create_payroll_run call, not just the "Create Payroll Run from This
-- Week" button on Timesheets - it's the same single RPC either way, and the reasoning holds for
-- both: Base Pay is already computed attendance-based off PayrollTimesheetEntries for every pay
-- type (see supabase_payroll_base_pay_days_worked.sql's header) - an employee with literally no
-- timesheet rows for the whole period already resolves to Base Pay = 0 either way (every day counts
-- as Undertime with nothing logged against it), so the old behavior just created a visible, useless
-- zero-pay line. This is a wash for pay: what changes is that line no longer shows up at all.
--
-- One edge case worth knowing: an employee with HasPaidRestDay = true who has ZERO timesheet
-- entries for the entire period (not just missing their rest day - literally nothing logged at all)
-- would previously have still gotten their flat one-day Paid Rest Day addition even with a $0 Base
-- Pay line; now they're skipped from the run entirely along with everyone else with no attendance
-- data, so that addition wouldn't post either. In practice this only matters if someone is on a
-- Paid Rest Day AND had zero days logged the whole period, which likely means the whole period was
-- something else (unpaid leave, not yet started, etc.) that the officer should look into rather
-- than have silently deducted or added to blindly.
--
-- Run this AFTER supabase_payroll_paid_rest_day_boolean.sql.
...
begin
  ...
  -- Skips anyone with zero timesheet entries in the period entirely - see this file's header.

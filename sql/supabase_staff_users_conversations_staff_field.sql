-- Adds a "Conversations Access" flag to StaffUsers, per: "can you add new permission per user to
-- be able to assign [GMA Conversations]." GMA Conversations (gma-conversations.html/
-- js/gmaConversations.js) was previously gated to Super User only (see the `if
-- (!session.isSuperUser)` check in js/gmaConversations.js's init()) with no assignment concept at
-- all inside the page. This flag widens that gate the same way Payroll Officer widens the payroll
-- pages - a plain additive tag, same shape as "PayrollOfficer"/"ProductionMember" (see
-- supabase_staff_users_payroll_officer_field.sql), NOT an exclusive lockdown like "Delivery
-- Team"/"Online Order Staff"/"Store Manager". A Conversations-flagged account keeps normal portal
-- access everywhere else, plus the ability to open gma-conversations.html.
--
-- Run this AFTER supabase_staff_users_store_manager_field.sql (layers on top of
-- admin_create_staff_user's/admin_update_staff_user's/admin_list_staff_users' current signatures)
-- - then apply the matching js/ changes (auth.js/nav.js/userSetup.js/gmaConversations.js) alongside
-- this.

alter table public."StaffUsers"
    add column if not exists "ConversationsStaff" boolean not null default false;

-- ---------------------------------------------------------------------------
-- verify_login: add is_conversations_staff to the returned columns so the portal session (see
-- attemptLogin/refreshPortalSession in js/auth.js) knows to unlock GMA Conversations.

drop function if exists public.verify_login(text, text);

create or replace function public.verify_login(p_username text, p_password text)
returns table(success boolean, display_name text, warehouse_name text, is_super_user boolean, is_sales_user boolean, is_serial_admin boolean, is_delivery_team boolean, is_online_order_staff boolean, is_production_member boolean, is_payroll_officer boolean, is_store_manager boolean, is_conversations_staff boolean, must_change_password boolean, message text)
language plpgsql
security definer
...
begin
  select "PasswordHash", "DisplayName"::text, "WarehouseName"::text, "SuperUser", "SalesUser", "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "ProductionMember", "PayrollOfficer", "StoreManager", "ConversationsStaff", "MustChangePassword", "IsActive", "LockedUntilUtc", "FailedAttempts"
    into v_password_hash, v_display_name, v_warehouse_name, v_super_user, v_sales_user, v_serial_admin, v_delivery_team, v_online_order_staff, v_production_member, v_payroll_officer, v_store_manager, v_conversations_staff, v_must_change_password, v_is_active, v_locked_until, v_failed_attempts
    from public."StaffUsers"
    where "Username" = p_username;

  if not found or not v_is_active then
    return query select false, null::text, null::text, false, false, false, false, false, false, false, false, false, false, 'Invalid username or password.'::text;
    return;
  end if;

  if v_locked_until is not null and v_locked_until > now() then
    return query select false, null::text, null::text, false, false, false, false, false, false, false, false, false, false, 'Account temporarily locked. Try again later.'::text;
    return;
  end if;
  ...
  if v_password_hash = crypt(p_password, v_password_hash) then
    update public."StaffUsers"
      ...
      where "Username" = p_username;
    return query select true, v_display_name, v_warehouse_name, coalesce(v_super_user, false), coalesce(v_sales_user, false), coalesce(v_serial_admin, false), coalesce(v_delivery_team, false), coalesce(v_online_order_staff, false), coalesce(v_production_member, false), coalesce(v_payroll_officer, false), coalesce(v_store_manager, false), coalesce(v_conversations_staff, false), coalesce(v_must_change_password, false), 'OK'::text;
  else
    update public."StaffUsers"
...
-- ---------------------------------------------------------------------------
-- admin_list_staff_users: add is_conversations_staff to User Setup's table.
...
-- ---------------------------------------------------------------------------
-- admin_create_staff_user / admin_update_staff_user: add p_is_conversations_staff (appended at the
-- end so every existing call site's other positional/named args are unaffected).

drop function if exists public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, boolean, text, boolean);
...
create or replace function public.admin_create_staff_user(
  ...
  p_is_conversations_staff boolean default false
)
...
begin
  ...
  insert into public."StaffUsers" (
    ...
    "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "ProductionMember", "PayrollOfficer", "StoreManager", "ConversationsStaff",
  ...
  values (
    ...
    coalesce(p_is_serial_admin, false), coalesce(p_is_delivery_team, false), coalesce(p_is_online_order_staff, false), coalesce(p_is_production_member, false), coalesce(p_is_payroll_officer, false), coalesce(p_is_store_manager, false), coalesce(p_is_conversations_staff, false),
...
drop function if exists public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, boolean, text, boolean);
  ...
  if p_new_password is not null and trim(p_new_password) <> '' then
    ...
    update public."StaffUsers"
      set "DisplayName" = nullif(trim(p_display_name), ''),
          ...
          "ConversationsStaff" = coalesce(p_is_conversations_staff, false),
...
grant execute on function public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, boolean, text, boolean, boolean) to anon;
grant execute on function public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, boolean, text, boolean, boolean) to anon;

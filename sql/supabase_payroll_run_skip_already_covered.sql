-- Per "if the employee already create a payroll run can you not include them on the next payroll
-- run with the same coverage" - creating a second run whose period overlaps one an employee is
-- already on (Draft or Finalized) would double-pay their Base Pay/Overtime for the same days, since
-- nothing else stops the same timesheet hours being read twice. Same precedent as skipping an
-- employee with no timesheet entries at all in the period (supabase_payroll_run_skip_no_timesheet.
-- sql) - one more NOT EXISTS on the same seeding insert, no interface change.
--
-- "Same coverage" = any overlap, not just an exact PeriodStart/PeriodEnd match - a run for 9/14-
-- 9/21 should still skip someone already on a 9/14-9/20 run. Checked against PayrollRunLines
-- regardless of that other run's Status (even a still-Draft run reserves that coverage) - if that
-- other run is later deleted, PayrollRunLines cascades away with it (on delete cascade,
-- supabase_payroll_tables.sql) and the employee becomes eligible for that period again
...
begin
  ...
  -- the period entirely, and anyone already on another run (Draft or Finalized) whose period
  -- overlaps this one.
  ...
  where su."IsActive" is true and su."PayCycle" = p_pay_cycle
    ...
    and not exists (
      select 1
      from public."PayrollRunLines" pl
      join public."PayrollRuns" pr on pr."RunID" = pl."RunID"
      where pl."Username" = su."Username"
        and pr."PeriodStart" <= p_period_end and pr."PeriodEnd" >= p_period_start
    );

  get diagnostics v_inserted = row_count;
  if v_inserted = 0 then
    delete from public."PayrollRuns" where "RunID" = v_run_id;
    raise exception 'No employees in the % pay cycle need a run for % to % - they either have no timesheet entries in this period or are already covered by another run for it.', p_pay_cycle, p_period_start, p_period_end;
  end if;

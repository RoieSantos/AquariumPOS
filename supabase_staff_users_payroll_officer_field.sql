-- Adds a "Payroll Officer" flag to StaffUsers, per direct request: "can I add a new field under
-- user setup boolean 'Payroll Officer' - meaning if this is true the user can see the payroll
-- setup / payroll."
--
-- Plain TAG flag, same shape as "SalesUser"/"SerialAdmin"/"ProductionMember" (see
-- supabase_staff_users_production_member_field.sql) - it does NOT confine the account to payroll
-- pages only (unlike "Delivery Team"/"Online Order Staff", which ARE exclusive lockdowns - see
-- supabase_staff_users_delivery_team_field.sql). A Payroll Officer keeps normal portal access
-- everywhere else, PLUS the ability to open payroll.html/payroll-run.html/payroll-setup.html/
-- payroll-ledger.html, which were previously gated to Super User only. Each of those four page
-- scripts (js/payroll.js, js/payrollRun.js, js/payrollSetup.js, js/payrollLedger.js) has its
-- `if (!session.isSuperUser)` gate widened to `if (!session.isSuperUser && !session.isPayrollOfficer)`
-- - not added to js/auth.js's requireAuth(), since (unlike Delivery Team/Online Order Staff) this
-- role doesn't need to be locked OUT of anything, only unlocked INTO payroll.
--
-- Run this AFTER supabase_staff_users_table.sql, supabase_staff_users_production_member_field.sql,
-- and supabase_staff_users_hr_profile_fields.sql (layers on top of admin_create_staff_user's/
-- admin_update_staff_user's current fuller signature).

alter table public."StaffUsers"
    add column if not exists "PayrollOfficer" boolean not null default false;

-- ---------------------------------------------------------------------------
-- verify_login: add is_payroll_officer to the returned columns so the portal session (see
-- attemptLogin/refreshPortalSession in js/auth.js) knows to unlock the Payroll pages.

drop function if exists public.verify_login(text, text);

create or replace function public.verify_login(p_username text, p_password text)
returns table(success boolean, display_name text, warehouse_name text, is_super_user boolean, is_sales_user boolean, is_serial_admin boolean, is_delivery_team boolean, is_online_order_staff boolean, is_production_member boolean, is_payroll_officer boolean, must_change_password boolean, message text)
language plpgsql
security definer
...
begin
  select "PasswordHash", "DisplayName"::text, "WarehouseName"::text, "SuperUser", "SalesUser", "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "ProductionMember", "PayrollOfficer", "MustChangePassword", "IsActive", "LockedUntilUtc", "FailedAttempts"
    into v_password_hash, v_display_name, v_warehouse_name, v_super_user, v_sales_user, v_serial_admin, v_delivery_team, v_online_order_staff, v_production_member, v_payroll_officer, v_must_change_password, v_is_active, v_locked_until, v_failed_attempts
    from public."StaffUsers"
    where "Username" = p_username;

  if not found or not v_is_active then
    return query select false, null::text, null::text, false, false, false, false, false, false, false, false, 'Invalid username or password.'::text;
    return;
  end if;

  if v_locked_until is not null and v_locked_until > now() then
    return query select false, null::text, null::text, false, false, false, false, false, false, false, false, 'Account temporarily locked. Try again later.'::text;
    return;
  end if;
  ...
  if v_password_hash = crypt(p_password, v_password_hash) then
    update public."StaffUsers"
      ...
      where "Username" = p_username;
    return query select true, v_display_name, v_warehouse_name, coalesce(v_super_user, false), coalesce(v_sales_user, false), coalesce(v_serial_admin, false), coalesce(v_delivery_team, false), coalesce(v_online_order_staff, false), coalesce(v_production_member, false), coalesce(v_payroll_officer, false), coalesce(v_must_change_password, false), 'OK'::text;
  else
    update public."StaffUsers"
...
-- ---------------------------------------------------------------------------
-- admin_list_staff_users: add is_payroll_officer to User Setup's table.
...
-- ---------------------------------------------------------------------------
-- admin_create_staff_user / admin_update_staff_user: add p_is_payroll_officer, appended at the
-- end of the current signature (same incremental-append pattern as every prior flag/field).

drop function if exists public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric);
...
drop function if exists public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric);
...
grant execute on function public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean) to anon;
grant execute on function public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean) to anon;

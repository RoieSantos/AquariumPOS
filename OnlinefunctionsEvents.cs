...
ORDER BY [RunningSerialNo]", connection);
            ...
            while (rdr.Read())
            {
                ...
                serialRecords.Add(new ItemSerialTrackingSupabaseRow(
                    ...
                    {
                        // RunningSerialNo is deliberately NOT sent to Supabase: it's Supabase's own
                        // "generated by default as identity" primary key there, while locally it's
                        // each store's own independent IDENTITY sequence (see the big comment above
                        // SyncItemSerialTrackingFromSupabaseAsync - matching is by SerialNo, never by
                        // this column). Two stores' local sequences overlap constantly (both start
                        // from 1), so pushing the local value as an explicit insert/update of
                        // Supabase's primary key was hitting "duplicate key value violates unique
                        // constraint ItemSerialTracking_pkey" as soon as a second store's serial
                        // landed on a RunningSerialNo Supabase had already assigned to someone else's
                        // row. Leaving it out lets Supabase's identity column assign its own value,
                        // same as SyncItemSerialTrackingFromSupabaseAsync's own INSERT already does.
                        ["SerialNo"] = serialNo,

-- Adds an "Online Order Staff" flag to StaffUsers, per direct request: "create me a field in the
-- user setup 'Online Order Staff' - when this is tick the user will only see Orders Printed that
-- to be Ship. They are the users have the ability to picture the item to-ship and auto send via
-- api to the customer."
--
-- Same exclusive-lockdown shape as "Delivery Team" (see
-- supabase_staff_users_delivery_team_field.sql) - an Online Order Staff account is confined to
-- online-orders.html/online-order-lines.html and nowhere else, enforced in js/auth.js's
-- requireAuth() (every authenticated page calls it, so this can't be bypassed by
-- bookmarking/typing a different URL directly). Unlike Delivery Team, this role doesn't get its
-- OWN new capability - the "To Ship" button + camera-capture-photo + auto-notify flow already
-- exists for any active staff on Online Orders (see supabase_online_order_portal_status_update.sql/
-- supabase_online_order_status_photo.sql) - this flag just locks an account down to ONLY that one
-- narrow job (Printed orders, nothing else in the portal) rather than granting a new ability.
-- online-orders.html itself hard-filters the list to status='Printed' for these accounts (see
-- js/onlineOrders.js) - not just a default filter staff can clear, since the whole point is they
-- shouldn't be browsing other statuses.
--
-- Run this AFTER supabase_staff_users_table.sql (layers on top of the same StaffUsers
-- table/functions, following the same pattern as "DeliveryTeam").

alter table public."StaffUsers"
    add column if not exists "OnlineOrderStaff" boolean not null default false;

-- ---------------------------------------------------------------------------
-- verify_login: add is_online_order_staff to the returned columns so the portal session (see
-- attemptLogin/refreshPortalSession in js/auth.js) knows to lock this account to online-orders.html.

drop function if exists public.verify_login(text, text);

create or replace function public.verify_login(p_username text, p_password text)
returns table(success boolean, display_name text, warehouse_name text, is_super_user boolean, is_sales_user boolean, is_serial_admin boolean, is_delivery_team boolean, is_online_order_staff boolean, must_change_password boolean, message text)
language plpgsql
security definer
...
begin
  select "PasswordHash", "DisplayName"::text, "WarehouseName"::text, "SuperUser", "SalesUser", "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "MustChangePassword", "IsActive", "LockedUntilUtc", "FailedAttempts"
    into v_password_hash, v_display_name, v_warehouse_name, v_super_user, v_sales_user, v_serial_admin, v_delivery_team, v_online_order_staff, v_must_change_password, v_is_active, v_locked_until, v_failed_attempts
    from public."StaffUsers"
    where "Username" = p_username;

  if not found or not v_is_active then
    return query select false, null::text, null::text, false, false, false, false, false, false, 'Invalid username or password.'::text;
    return;
  end if;

  if v_locked_until is not null and v_locked_until > now() then
    return query select false, null::text, null::text, false, false, false, false, false, false, 'Account temporarily locked. Try again later.'::text;
    return;
  end if;
  ...
  if v_password_hash = crypt(p_password, v_password_hash) then
    update public."StaffUsers"
      ...
      where "Username" = p_username;
    return query select true, v_display_name, v_warehouse_name, coalesce(v_super_user, false), coalesce(v_sales_user, false), coalesce(v_serial_admin, false), coalesce(v_delivery_team, false), coalesce(v_online_order_staff, false), coalesce(v_must_change_password, false), 'OK'::text;
  else
    update public."StaffUsers"
...
-- ---------------------------------------------------------------------------
-- admin_list_staff_users: add is_online_order_staff to User Setup's table.
...
begin
  ...
  return query
    select "Username"::text, "DisplayName"::text, "WarehouseName"::text, "SuperUser", "SalesUser", "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "MonthlySalesTarget", "MustChangePassword", "IsActive", "CreatedAtUtc", "LastLoginAtUtc",
           count(*) over()
    from public."StaffUsers"
    order by "Username"
...
-- ---------------------------------------------------------------------------
-- admin_create_staff_user / admin_update_staff_user: add p_is_online_order_staff.
...
grant execute on function public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean) to anon;

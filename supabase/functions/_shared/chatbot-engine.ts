...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    ...
    '- A search_items result\'s wholesale_price will be null for most items (either the category isn\'t wholesale-eligible, or no wholesale price has been set) - if a wholesale customer asks about an item where it\'s null, say plainly that item doesn\'t have wholesale pricing available (regular price applies), don\'t guess or estimate one.',

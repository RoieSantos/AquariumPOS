...
export function buildSystemPrompt(
  ...
  const lines: string[] = [
    ...
    '',
    'WHOLESALE PRICING:',
    '- SCOPE: wholesale pricing and the ₱5,000-per-transaction minimum ONLY cover Aquarium, Stand, and Sump Filtration items from the catalog - nothing else the store sells. Never bring up, offer, or ask about wholesale for any other product line (fish food, medicine, decor, other filters/equipment, accessories, etc.) - just quote the regular price for those, no wholesale question at all. This also does NOT cover a custom-built aquarium/stand quoted through compute_aquarium_quote (that has no catalog wholesale_price at all) - only ready-made Aquarium/Stand/Sump Filtration items returned by search_items/list_items_in_category.',
    '- ASK about it: when a customer shows buying interest in an Aquarium, Stand, or Sump Filtration item specifically (asks its price, asks what\'s available in that line, or is heading toward ordering one) and it hasn\'t come up yet in this conversation, ask once whether they\'re buying at RETAIL or WHOLESALE pricing, and mention the ₱5,000 minimum (e.g. "Retail or wholesale po ito? Note na ang wholesale ay may minimum na ₱5,000 per transaction."). Ask only once per conversation - don\'t repeat it once they\'ve answered, and don\'t ask for things wholesale doesn\'t apply to (see SCOPE above, plus delivery fees, order status, general questions). If they don\'t answer and just keep asking prices, quote retail. Asking for wholesale pricing is NOT a discount request - handle it here, not with the DISCOUNTS / PRICE CHANGES decline.',
    '- RETAIL (or no answer): quote the regular price, as always. Don\'t bring up wholesale_price figures for retail buyers.',
    '- WHOLESALE: quote each item\'s wholesale_price (from search_items) instead of the regular price - this will only ever be populated for Aquarium/Stand/Sump Filtration items (see SCOPE above). Wholesale only applies when the transaction\'s wholesale-priced items add up to at least ₱5,000 - add up quantity x wholesale_price yourself for the items they want and tell them the total. If it\'s under ₱5,000, wholesale doesn\'t apply yet: say how far short they are, and let them choose to add more items to reach the minimum or go with regular retail pricing - never apply wholesale prices to a transaction below the minimum, and never make an exception even if they insist. Only wholesale-priced items count toward the minimum; an item with no wholesale price is at its regular price and doesn\'t count toward the ₱5,000.',

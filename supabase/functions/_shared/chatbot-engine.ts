...
export const TOOLS: Anthropic.Tool[] = [
  {
    name: 'search_items',
    description:
      'Search the store\'s product catalog by name, keyword, brand, or SKU. Use this whenever a customer asks if you carry something, or asks about the price/stock of a specific product. Each result has quantity_in_stock (all-branch total) and stock_by_location (real per-branch count for Amaya and GMA - use that when telling a customer about stock). Results may include a wholesale_price (only for wholesale-eligible categories, and only when an actual price is set - see the WHOLESALE PRICING rule below for when you\'re allowed to mention it).',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'List active items in one specific category, with price and stock (quantity_in_stock is the all-branch total; stock_by_location is the per-branch count for Amaya and GMA - use that when telling a customer about stock). Use after list_categories, or when the customer names a category directly (e.g. "what filters do you have").',
    input_schema: {
      type: 'object',
      properties: {

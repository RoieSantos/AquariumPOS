-- Paid Rest Day: distinguishes employees who have a paid weekly rest day (their absence
-- calculation excuses that one weekday every period, even if left blank) from employees who
-- don't (every blank day still counts as absent, per supabase_payroll_absent_day_deduction.sql).
--
-- Adds StaffUsers."PaidRestDay" - a nullable day-of-week number (0 = Sunday ... 6 = Saturday,
-- matching Postgres's extract(dow from date)). Null means "no paid rest day" - unchanged behavior,
-- every calendar day in the period is still an expected workday. A non-null value excludes that
-- one weekday from the Overtime/Absence day-walk entirely in admin_create_payroll_run, so it never
-- generates an Absence deduction (or an Overtime addition, if they happened to log hours on it -
-- an edge case, not the point of this feature).
--
-- Hourly employees are unaffected - they have no Absence deduction to begin with.
--
-- Run this AFTER supabase_payroll_absence_rate_fix.sql.

alter table public."StaffUsers"
    add column if not exists "PaidRestDay" int null;

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'CK_StaffUsers_PaidRestDay') then
    alter table public."StaffUsers"
      add constraint "CK_StaffUsers_PaidRestDay" check ("PaidRestDay" is null or "PaidRestDay" between 0 and 6);
  end if;
end $$;

-- ---------------------------------------------------------------------------
-- admin_list_staff_users: add paid_rest_day to User Setup's table.
...
begin
  ...
  return query
    select "Username"::text, "DisplayName"::text, "WarehouseName"::text, "SuperUser", "SalesUser", "SerialAdmin", "DeliveryTeam", "OnlineOrderStaff", "ProductionMember", "PayrollOfficer", "MonthlySalesTarget", "MustChangePassword", "IsActive", "CreatedAtUtc", "LastLoginAtUtc",
           "Position"::text, "HomeAddress"::text, "Birthdate", "PhoneNumber"::text, "PaymentMethod"::text, "HireDate", "PayCycle"::text, "MonthlySalary", "PayType"::text, "DailyRate", "PaidRestDay",
           count(*) over()
...
-- ---------------------------------------------------------------------------
-- admin_create_staff_user / admin_update_staff_user: add p_paid_rest_day, appended at the end.

drop function if exists public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric);
...
begin
  ...
  insert into public."StaffUsers" (
    ...
    "Position", "HomeAddress", "Birthdate", "PhoneNumber", "PaymentMethod", "HireDate", "PayCycle", "MonthlySalary", "PayType", "DailyRate", "PaidRestDay"
  )
  values (
    ...
    nullif(trim(p_position), ''), nullif(trim(p_home_address), ''), p_birthdate, nullif(trim(p_phone_number), ''), p_payment_method, p_hire_date, p_pay_cycle, coalesce(p_monthly_salary, 0), coalesce(p_pay_type, 'Salary'), coalesce(p_daily_rate, 0), p_paid_rest_day
  );
...
drop function if exists public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric);
...
-- same paid_rest_day field, surfaced in Payroll Setup too.
...
begin
  ...
  return query
    select "Username"::text, "DisplayName"::text, "IsActive", "PayCycle"::text, "MonthlySalary", "PaymentMethod"::text, "PayType"::text, "DailyRate", "PaidRestDay"
    from public."StaffUsers"
    order by "DisplayName", "Username";
end;
$$;

drop function if exists public.admin_update_payroll_profile(text, text, text, text, numeric, boolean, text, text, numeric);
...
drop function if exists public.admin_create_payroll_employee(text, text, text, text, text, text, text, date, text, text, date, text, numeric, text, numeric);
...
-- ---------------------------------------------------------------------------
-- admin_create_payroll_run: same signature - a Salary employee's designated PaidRestDay weekday is
-- excluded from the Overtime/Absence day-walk entirely, so it never generates an Absence deduction.
...
grant execute on function public.admin_create_staff_user(text, text, text, text, text, text, boolean, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, int) to anon;
grant execute on function public.admin_update_staff_user(text, text, text, text, text, boolean, boolean, text, boolean, numeric, boolean, boolean, boolean, boolean, boolean, text, text, date, text, text, date, text, numeric, boolean, text, numeric, int) to anon;
grant execute on function public.admin_list_payroll_employees(text, text) to anon;
grant execute on function public.admin_update_payroll_profile(text, text, text, text, numeric, boolean, text, text, numeric, int) to anon;
grant execute on function public.admin_create_payroll_employee(text, text, text, text, text, text, text, date, text, text, date, text, numeric, text, numeric, int) to anon;
